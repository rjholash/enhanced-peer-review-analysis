...
"""
Group peer review report generator with reliability metrics.
"""
...
def create_group_peer_review_reports():
    """Create individual group peer review reports with reliability metrics."""
    ...
    messagebox.showinfo(
        ...
        'Please select a destination folder for the group peer review HTML reports.'
    )
    ...
    for group, data in grouped:
        try:
            ...
            # Group component only: peer review of the video contributes 35% of the assignment.
            peer_review_component = round(mark * 0.35, 2)
...
<html>
<head>
    <title>Group {group} Peer Review Report</title>
    <style>
        body {{
            max-width: 900px;
            margin: 0 auto;
...
<body>
    <div class="header">
        <h1>Group {group} Peer Review Report</h1>
        <p>Peer review analysis with reliability metrics</p>
    </div>

    <div class="metric-box">
        <p><strong>How reliability affected grading:</strong> These reliability statistics are included to help interpret the feedback, but inconsistent reviewing by other students was not used to directly reduce your group's score.</p>
    </div>

    <div class="metric-box">
        <h2>Final Scores</h2>
        <p class="score-highlight">Peer Review Grade: {mark}%</p>
        <p class="score-highlight">Peer Review Component (35%): {peer_review_component}/35</p>
        <p><strong>Important:</strong> Reliability metrics are shown for transparency only. Variation in how other students reviewed your work did not directly lower your group's mark in this report.</p>
    </div>
    ...
    <div class="metric-box">
        ...
        <p><strong>Grade Calculation:</strong> Your peer review score ({mark}%) × 35% = {peer_review_component}/35 for the group peer review portion of the assignment.</p>
    </div>

    <footer style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #ddd; color: #666; text-align: center;">
        <p>Generated by Peer Review Analysis System | Report includes reliability metrics</p>
    </footer>
</body>
</html>"""

            # Save the file
            file_path = f"{folder_path}/Group_{group}Peer_Review_Report.html"
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(html_content)

            print(f"Group {group} peer review report saved at {file_path}")
            
        except Exception as e:
            print(f"Error processing Group {group}: {str(e)}")
            continue

    messagebox.showinfo("Success", f"Group peer review reports generated successfully.\n\nSaved to: {folder_path}")


if __name__ == "__main__":
    create_group_peer_review_reports()
